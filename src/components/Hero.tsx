...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // تقليل الجزيئات في الجوال لتحسين الأداء والسرعة
    ...
    const animate = () => {
      ...
      // إخفاء الخطوط الواصلة في الجوال لتحسين الأداء
    ...
    >
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 1.3 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <motion.a href="#products" whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }} className="btn-primary text-base px-10 py-4 w-full sm:w-auto max-w-xs">
            استكشفي المجموعة
          </motion.a>
        </motion.div>
      ...
      {/* مؤشر التمرير (يختفي في الموبايل لتوفير مساحة) */}
